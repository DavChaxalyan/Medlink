    ...
    <div class="relative overflow-hidden rounded-lg">
      <div class="flex transition-transform duration-700 ease-in-out"
        ...
        <div v-for="(slide, index) in slides" :key="index" class="min-w-full flex-shrink-0 relative mr-[80px]">
          <img :src="slide.image" alt="Slide image" class="w-full h-[640px] object-cover rounded-lg"
            style="clip-path: polygon(30% 7%, 100% 7%, 100% 60%, 100% 100%, 0 100%, 0 0, 25% 0);">
          <div
            class="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black via-transparent to-transparent text-white">
            <h2
              class="w-[490px] mb-[16px] font-medium text-[32px] leading-[38.4px] tracking[-0.02em] text-left text-[#FFFFFF]">
              {{ slide.title }}</h2>
            <p class="w-[780px] font-medium text-[24px] leading-[28.8px] text-left text-[#FFFFFFA3]">{{
              slide.description }}</p>
